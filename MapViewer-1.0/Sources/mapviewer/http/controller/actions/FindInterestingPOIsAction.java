...
public class FindInterestingPOIsAction extends DefaultAction {
	...
	/* Do find interesting POIs. */
	return doFindInterestingPOIs(mapping, userID, startIndex, count,
		request);
    }

    private ActionForward doFindInterestingPOIs(ActionMapping mapping,
	    String userID, int startIndex, int count, HttpServletRequest request)
	    throws InternalErrorException {
	/* Find interesting POIs by user. */
	SearchFacadeDelegate searchFacadeDelegate = SearchFacadeDelegateFactory
		.getDelegate();

	POIChunkTO POIs;
	POIs = searchFacadeDelegate.findInterestingPOIsByUser(userID,
		startIndex, count);
	...
	return mapping.findForward("FindInterestingPOIsContent");
    }
