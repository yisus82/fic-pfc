...
public class FindInterestingMapsAction extends DefaultAction {
	...
	/* Do find interesting maps. */
	return doFindInterestingMaps(mapping, userID, startIndex, count,
		request);
    }

    private ActionForward doFindInterestingMaps(ActionMapping mapping,
	    String userID, int startIndex, int count, HttpServletRequest request)
	    throws InternalErrorException {
	/* Find interesting maps by user. */
	SearchFacadeDelegate searchFacadeDelegate = SearchFacadeDelegateFactory
		.getDelegate();

	MapChunkTO maps;
	maps = searchFacadeDelegate.findInterestingMapsByUser(userID,
		startIndex, count);
	...
	return mapping.findForward("FindInterestingMapsContent");
    }
